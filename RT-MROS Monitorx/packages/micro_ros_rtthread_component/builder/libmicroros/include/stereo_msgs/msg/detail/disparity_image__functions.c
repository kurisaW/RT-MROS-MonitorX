  ...
  stereo_msgs__msg__DisparityImage__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    stereo_msgs__msg__DisparityImage * data =
      (stereo_msgs__msg__DisparityImage *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!stereo_msgs__msg__DisparityImage__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          stereo_msgs__msg__DisparityImage__fini(&data[i]);
        }
        free(data);
        return false;
      }
