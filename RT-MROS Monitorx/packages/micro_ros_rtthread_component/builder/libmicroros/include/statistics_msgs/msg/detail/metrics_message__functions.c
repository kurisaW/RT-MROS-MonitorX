  ...
  statistics_msgs__msg__MetricsMessage__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    statistics_msgs__msg__MetricsMessage * data =
      (statistics_msgs__msg__MetricsMessage *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!statistics_msgs__msg__MetricsMessage__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          statistics_msgs__msg__MetricsMessage__fini(&data[i]);
        }
        free(data);
        return false;
      }
