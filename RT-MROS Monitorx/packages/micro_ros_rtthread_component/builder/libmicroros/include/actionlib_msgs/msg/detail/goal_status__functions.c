  ...
  actionlib_msgs__msg__GoalStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    actionlib_msgs__msg__GoalStatus * data =
      (actionlib_msgs__msg__GoalStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!actionlib_msgs__msg__GoalStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          actionlib_msgs__msg__GoalStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
