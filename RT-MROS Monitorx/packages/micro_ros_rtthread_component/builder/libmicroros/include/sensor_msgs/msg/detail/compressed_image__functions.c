  ...
  sensor_msgs__msg__CompressedImage__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sensor_msgs__msg__CompressedImage * data =
      (sensor_msgs__msg__CompressedImage *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sensor_msgs__msg__CompressedImage__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sensor_msgs__msg__CompressedImage__fini(&data[i]);
        }
        free(data);
        return false;
      }
