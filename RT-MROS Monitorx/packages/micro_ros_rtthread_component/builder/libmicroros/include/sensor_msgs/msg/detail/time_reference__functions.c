  ...
  sensor_msgs__msg__TimeReference__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sensor_msgs__msg__TimeReference * data =
      (sensor_msgs__msg__TimeReference *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sensor_msgs__msg__TimeReference__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sensor_msgs__msg__TimeReference__fini(&data[i]);
        }
        free(data);
        return false;
      }
