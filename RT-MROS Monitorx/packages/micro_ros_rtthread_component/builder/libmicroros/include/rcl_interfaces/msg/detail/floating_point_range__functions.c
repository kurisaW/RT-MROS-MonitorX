  ...
  rcl_interfaces__msg__FloatingPointRange__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rcl_interfaces__msg__FloatingPointRange * data =
      (rcl_interfaces__msg__FloatingPointRange *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rcl_interfaces__msg__FloatingPointRange__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rcl_interfaces__msg__FloatingPointRange__fini(&data[i]);
        }
        free(data);
        return false;
      }
