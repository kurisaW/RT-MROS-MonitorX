  ...
  rcl_interfaces__msg__ParameterEventDescriptors__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rcl_interfaces__msg__ParameterEventDescriptors * data =
      (rcl_interfaces__msg__ParameterEventDescriptors *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rcl_interfaces__msg__ParameterEventDescriptors__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rcl_interfaces__msg__ParameterEventDescriptors__fini(&data[i]);
        }
        free(data);
        return false;
      }
