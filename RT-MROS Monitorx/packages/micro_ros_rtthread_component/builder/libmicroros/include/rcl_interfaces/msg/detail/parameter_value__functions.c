  ...
  rcl_interfaces__msg__ParameterValue__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rcl_interfaces__msg__ParameterValue * data =
      (rcl_interfaces__msg__ParameterValue *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rcl_interfaces__msg__ParameterValue__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rcl_interfaces__msg__ParameterValue__fini(&data[i]);
        }
        free(data);
        return false;
      }
