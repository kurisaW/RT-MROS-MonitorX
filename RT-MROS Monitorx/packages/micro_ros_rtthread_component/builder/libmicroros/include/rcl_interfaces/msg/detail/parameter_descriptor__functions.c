  ...
  rcl_interfaces__msg__ParameterDescriptor__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rcl_interfaces__msg__ParameterDescriptor * data =
      (rcl_interfaces__msg__ParameterDescriptor *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rcl_interfaces__msg__ParameterDescriptor__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rcl_interfaces__msg__ParameterDescriptor__fini(&data[i]);
        }
        free(data);
        return false;
      }
