  ...
  unique_identifier_msgs__msg__UUID__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    unique_identifier_msgs__msg__UUID * data =
      (unique_identifier_msgs__msg__UUID *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!unique_identifier_msgs__msg__UUID__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          unique_identifier_msgs__msg__UUID__fini(&data[i]);
        }
        free(data);
        return false;
      }
