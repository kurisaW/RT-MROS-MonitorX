  ...
  action_msgs__msg__GoalStatusArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    action_msgs__msg__GoalStatusArray * data =
      (action_msgs__msg__GoalStatusArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!action_msgs__msg__GoalStatusArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          action_msgs__msg__GoalStatusArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
