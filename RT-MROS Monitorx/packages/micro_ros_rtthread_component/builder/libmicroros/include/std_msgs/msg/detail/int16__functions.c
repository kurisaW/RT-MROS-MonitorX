...
std_msgs__msg__Int16__Sequence__init(std_msgs__msg__Int16__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (std_msgs__msg__Int16 *)allocator.zero_allocate(size, sizeof(std_msgs__msg__Int16), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  std_msgs__msg__Int16__Sequence * array = (std_msgs__msg__Int16__Sequence *)allocator.allocate(sizeof(std_msgs__msg__Int16__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
...
std_msgs__msg__Int16__Sequence__are_equal(const std_msgs__msg__Int16__Sequence * lhs, const std_msgs__msg__Int16__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!std_msgs__msg__Int16__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  std_msgs__msg__Int16__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(std_msgs__msg__Int16);
    std_msgs__msg__Int16 * data =
      (std_msgs__msg__Int16 *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__Int16__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          std_msgs__msg__Int16__fini(&data[i]);
        }
        free(data);
        return false;
      }
