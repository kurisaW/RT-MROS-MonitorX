...
std_msgs__msg__Bool__Sequence__init(std_msgs__msg__Bool__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (std_msgs__msg__Bool *)allocator.zero_allocate(size, sizeof(std_msgs__msg__Bool), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  std_msgs__msg__Bool__Sequence * array = (std_msgs__msg__Bool__Sequence *)allocator.allocate(sizeof(std_msgs__msg__Bool__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
...
std_msgs__msg__Bool__Sequence__are_equal(const std_msgs__msg__Bool__Sequence * lhs, const std_msgs__msg__Bool__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!std_msgs__msg__Bool__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  std_msgs__msg__Bool__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(std_msgs__msg__Bool);
    std_msgs__msg__Bool * data =
      (std_msgs__msg__Bool *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__Bool__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          std_msgs__msg__Bool__fini(&data[i]);
        }
        free(data);
        return false;
      }
