  ...
  std_msgs__msg__UInt8MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__UInt8MultiArray * data =
      (std_msgs__msg__UInt8MultiArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__UInt8MultiArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          std_msgs__msg__UInt8MultiArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
