  ...
  std_msgs__msg__ByteMultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__ByteMultiArray * data =
      (std_msgs__msg__ByteMultiArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__ByteMultiArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          std_msgs__msg__ByteMultiArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
