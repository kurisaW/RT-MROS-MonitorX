  ...
  trajectory_msgs__msg__MultiDOFJointTrajectoryPoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    trajectory_msgs__msg__MultiDOFJointTrajectoryPoint * data =
      (trajectory_msgs__msg__MultiDOFJointTrajectoryPoint *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!trajectory_msgs__msg__MultiDOFJointTrajectoryPoint__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          trajectory_msgs__msg__MultiDOFJointTrajectoryPoint__fini(&data[i]);
        }
        free(data);
        return false;
      }
