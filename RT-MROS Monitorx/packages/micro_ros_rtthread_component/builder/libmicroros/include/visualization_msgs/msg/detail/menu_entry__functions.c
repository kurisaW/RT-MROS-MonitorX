  ...
  visualization_msgs__msg__MenuEntry__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    visualization_msgs__msg__MenuEntry * data =
      (visualization_msgs__msg__MenuEntry *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!visualization_msgs__msg__MenuEntry__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          visualization_msgs__msg__MenuEntry__fini(&data[i]);
        }
        free(data);
        return false;
      }
