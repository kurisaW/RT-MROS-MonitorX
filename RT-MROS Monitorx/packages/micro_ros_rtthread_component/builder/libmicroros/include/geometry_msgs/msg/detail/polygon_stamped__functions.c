  ...
  geometry_msgs__msg__PolygonStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    geometry_msgs__msg__PolygonStamped * data =
      (geometry_msgs__msg__PolygonStamped *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!geometry_msgs__msg__PolygonStamped__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          geometry_msgs__msg__PolygonStamped__fini(&data[i]);
        }
        free(data);
        return false;
      }
