  ...
  geometry_msgs__msg__Transform__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    geometry_msgs__msg__Transform * data =
      (geometry_msgs__msg__Transform *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!geometry_msgs__msg__Transform__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          geometry_msgs__msg__Transform__fini(&data[i]);
        }
        free(data);
        return false;
      }
