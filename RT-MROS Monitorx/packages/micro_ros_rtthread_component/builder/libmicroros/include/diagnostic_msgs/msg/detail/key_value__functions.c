  ...
  diagnostic_msgs__msg__KeyValue__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    diagnostic_msgs__msg__KeyValue * data =
      (diagnostic_msgs__msg__KeyValue *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!diagnostic_msgs__msg__KeyValue__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          diagnostic_msgs__msg__KeyValue__fini(&data[i]);
        }
        free(data);
        return false;
      }
