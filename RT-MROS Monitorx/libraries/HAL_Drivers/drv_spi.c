...
static struct ra_spi_handle spi_handle[] =
{
    ...
    {.bus_name = "spi3", .spi_ctrl_t = &g_spi3_ctrl, .spi_cfg_t = &g_spi3_cfg,},
#endif
};
...
void spi3_callback(spi_callback_args_t *p_args)
{
    ...
    if (SPI_EVENT_TRANSFER_COMPLETE == p_args->event)
    {
        rt_event_send(&complete_event, RA_SPI3_EVENT);
    }
    rt_interrupt_leave();
}
...
#else
    R_SPI_CalculateBitrate(spi_dev->rt_spi_cfg_t->max_hz, &spi_cfg.clk_div);
#endif
